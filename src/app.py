...
first_fig_x.update_layout(xaxis_title="y",
                          margin=dict(l=0, r=20, t=20, b=10), height=400,
...
card_3 = dbc.Card([
    dbc.CardBody([
        ...
        html.P(
            'You can also plot something like the Ackley function which looks like this:'),
        html.P(
            '-20*exp(-0.2*sqrt(0.5*(x**2+y**2)))-exp(0.5*(cos(2*3.14*x)+cos(2*3.14*y)))+20'),
    ])
], style={'marginTop': '10px'})
...
app.layout = html.Div([
    dbc.NavbarSimple(
        children=[
            dbc.Col([
                dbc.Row([
                    html.H4('Super Plotter 4.5', style={
